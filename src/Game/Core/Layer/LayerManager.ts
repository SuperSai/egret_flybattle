class LayerManager extends BaseClass {
	...
	private createAllLayers(): void {
		for (let i: number = 0; i < this.layerCount; i++) {
	...
	public addToLayer(display: egret.DisplayObject, layerType: number, center: boolean = false): void {
		let layer: DisplayLayer = this.getLayerByType(layerType);
		layer.addChild(display);
		if (center) {
			App.DisplayUtils.centerObjectToStage(display, true);
		}
	}
	...
	public resize(): void {
		...
		let width: number = App.StageUtils.stage.stageWidth;
		let height: number = App.StageUtils.stage.stageHeight;
		for (let i: number = 0; i < this._layers.length; i++) {
