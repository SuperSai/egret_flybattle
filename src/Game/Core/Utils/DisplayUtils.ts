...
class DisplayUtils extends BaseClass {
    ...
    public textDotAnimation(label: any): void {
        ...
        TweenLite.killTweensOf(label, self);
        let tl: TimelineMax = new TimelineMax({ repeat: -1 });
        tl.add(TweenLite.to(label, 0.5, { onComplete: () => { label.text = mainText + "." } }));
        tl.add(TweenLite.to(label, 0.5, { onComplete: () => { label.text = mainText + ".." } }));
        tl.add(TweenLite.to(label, 0.5, { onComplete: () => { label.text = mainText + "..." } }));
        tl.add(TweenLite.to(label, 0.5, { onComplete: () => { label.text = mainText } }));
    ...
    /** 移除数组中的所有数据 */
    public removeArrayAllChilder(array: any[]): void {
    ...
    public addToStageCenter(display: egret.DisplayObject, ignoreDisplaySize: boolean = false): void {
        App.StageUtils.stage.addChild(display);
        this.centerObjectToStage(display, ignoreDisplaySize);
    }

    public centerObjectToStage(display: egret.DisplayObject, ignoreDisplaySize: boolean = false): void {
        if (ignoreDisplaySize) {
            display.x = App.StageUtils.stage.stageWidth / 2;
            display.y = App.StageUtils.stage.stageHeight / 2;
        } else {
            display.x = (App.StageUtils.stage.stageWidth - display.width) / 2;
            display.y = (App.StageUtils.stage.stageHeight - display.height) / 2;
        }
    }
    ...
    */
    public layoutDisplayWithInnerRect(com: egret.DisplayObject, innerRect: InnerRectangle, width: number = 0, height: number = 0): void {
        ...
        if (width == 0 && height == 0) {
            ...
            width = App.StageUtils.stage.stageWidth;
            height = App.StageUtils.stage.stageHeight;
        }
    ...
    public addAsyncBimapToContainer(path: string, container: egret.DisplayObjectContainer) {
    ...
    public getAsyncBimapToHolder(path: string, holder: any) {
        RES.getResByUrl(path, () => {
            let bitmapData = RES.getRes(path);
            holder.display = new egret.Bitmap(bitmapData);
        }, this, RES.ResourceItem.TYPE_IMAGE);
    }
    ...
    */
    ...
    public isInViewArea(rect: RotatedRectangle, position: egret.Point): boolean {
        for (let i: number = 0; i < rect.points.length; i++) {
            if (World.Instance.worldRect.contains(rect.points[i].x, rect.points[i].y)) return true;
        }
        return false;
    }
}
